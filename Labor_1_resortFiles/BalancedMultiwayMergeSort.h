...
class BalancedMultiwayMergeSort {
    ...
    int GetDirectotyNumberOfFiles(const std::string& dirName) { 
    ...
    protected: // RUNS
    // Prepares runs controlling what file is the output one to be merged into
    void PrepareRun(int i) { 
        auto j = 0;
        while(!IsEndOfFiles()) {
            GoToNextChunk();
            Run(m_vOutput[j]);
    ...
    // Creates a run to merge one chunk from each of the input files into the single chunk of the output file
    void Run(std::fstream& outFile) { 
        InitMergeArray();
        while(!IsEndOfChunks()) {
    ...
    // Creates last run where all the files are merger into the resulting one
    void LastRun() { 
    ...
    // Initializes merge array of size of the number of ways
    void InitMergeArray() {  
        for(auto i = 0;i<m_iNumWays;i++) {
            if(!m_vInput[i].eof()) {
